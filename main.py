...
class AddExpenseScreen(MDApp):
    ...
    def submit_expense(self):
        ...
        # workbook.save(file_path)
        # ---------------------------------------
        self.upload_to_google_sheets(date, expense_type,subtype, description, amount,SCOPES,SERVICE_ACCOUNT_FILE,spreadsheet_id)
    ...
    def load_expenses(self):
        ...
        # Populate Expenses
        self.display_expenses(grid,SCOPES,SERVICE_ACCOUNT_FILE,spreadsheet_id,CREDS)

    def upload_to_google_sheets(self, date, expense_type, subtype, description, amount,SCOPES,SERVICE_ACCOUNT_FILE,spreadsheet_id):
        print("upload_to_google_sheets")
        print("SERVICE_ACCOUNT_FILE",SERVICE_ACCOUNT_FILE,"SCOPES",SCOPES,"spreadsheet_id",spreadsheet_id)
...
# -------------------------------------------------------------------------------------------------------------------4
    ...
    def display_expenses(self, grid,SCOPES,SERVICE_ACCOUNT_FILE,spreadsheet_id,CREDS):
        print("display_expenses")
        print("SERVICE_ACCOUNT_FILE",SERVICE_ACCOUNT_FILE,"SCOPES",SCOPES,"spreadsheet_id",spreadsheet_id,"CREDS",CREDS)
        self.clear_grid(grid)
        # Define the SCOPES and path to your service account credentials
        SCOPES = [SCOPES]
        ...
        try:
            ...
            if current_month_year in sheet_names:
                ...
                # Clear the existing total label if it exists
                # for widget in self.screen.ids.expense_list_viewlayout.children[:]:
                #     if isinstance(widget, MDLabel) and "Total:" in widget.text:
                #         self.screen.ids.expense_list_viewlayout.remove_widget(widget)
                total_amount = 0.0  
                for row in values:
                    try:
